...
GoRouter createRouter({String? initialLink}) {
  // Récupérer le token depuis le lien profond (si présent)
  String? token;
  if (initialLink != null && initialLink.startsWith('helpneighbor://reinitialiser')) {
    final uri = Uri.parse(initialLink);
    token = uri.queryParameters['token'];
  }

  print('🔧 createRouter initialLink=$initialLink, token=$token, initialLocation=${token != null ? '/reinitialiser-mot-de-passe?token=$token' : '/connexion'}');

  return GoRouter(
    initialLocation: token != null ? '/reinitialiser-mot-de-passe?token=$token' : '/connexion',
    routes: [
      // ✅ Route racine pour gérer les liens profonds avec paramètre token
      GoRoute(
        path: '/',
        redirect: (context, state) {
          final tokenParam = state.uri.queryParameters['token'];
          if (tokenParam != null && tokenParam.isNotEmpty) {
            return '/reinitialiser-mot-de-passe?token=$tokenParam';
          }
          return '/connexion';
        },
      ),
